...
const enviarTexto = (cfg: Record<string, any>, para: string, texto: string) =>
  chamarMeta(cfg, { to: para, type: 'text', text: { preview_url: true, body: texto.slice(0, 4000) } });
const enviarImagem = (cfg: Record<string, any>, para: string, link: string, legenda: string) =>
  chamarMeta(cfg, { to: para, type: 'image', image: { link, caption: legenda.slice(0, 1000) } });
...
/** As regras da tela Conversas: ligada, modo teste, horário, conversa pausada, quem atender. */
...
// ---------------------------------------------------------------- mensagem chegando
async function atender(db: DB, empresa: { id: string; nome: string }, corpo: any) {
  const cfg = await configDa(db, empresa.id);
  if (!cfg.wa_configurado || !cfg.wa_token) return;
  ...
  for (const entrada of corpo?.entry ?? []) for (const mudanca of entrada?.changes ?? []) {
    ...
    for (const msg of v.messages ?? []) {
      ...
      // vira lead (ou acha o que já existe) e liga na conversa
      ...
      if (['image', 'audio', 'document', 'video', 'sticker'].includes(msg.type) && !msg.caption) continue; // IA só lê texto
      const motivo = await podeResponder(db, cfg, conversa);
      if (motivo) { console.log(`sem resposta (${motivo})`); continue; }
      if (!iaConfigurada()) continue;

      await responderComIA(db, empresa, cfg, { ...conversa, lead_id: lead.id }, lead.nome || nome);
    }
  }
}

async function responderComIA(db: DB, empresa: { id: string; nome: string }, cfg: Record<string, any>, conversa: any, nome: string) {
  ...
  if (m.texto) {
    for (const parte of m.texto.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean)) await enviarTexto(cfg, tel, parte);
  ...
  for (const imv of escolhidos) {
    ...
    try {
      if (imv.fotos?.[0]) await enviarImagem(cfg, tel, imv.fotos[0], legenda);
      else await enviarTexto(cfg, tel, legenda);
...
async function retomadas(db: DB) {
  const a = agoraSP();
  if (a.getDay() === 0 || a.getDay() === 6 || a.getHours() < 9 || a.getHours() >= 19) return { enviadas: 0, motivo: 'fora do horário comercial' };
  ...
  for (const emp of empresas ?? []) {
    ...
    if (!cfg.bot_ligado || !cfg.wa_configurado || !cfg.wa_token) continue;
...
Deno.serve(responder(async (req) => {
  ...
  if (corpo.acao === 'enviar') {
    ...
    if (!cfg.wa_configurado || !cfg.wa_token) throw new ErroTela('Ligue o WhatsApp oficial em Ajustes primeiro.');
    await enviarTexto(cfg, c.telefone, texto);
